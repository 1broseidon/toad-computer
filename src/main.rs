use toad_computer::{App, Config, boot, serve, viewer};
...
fn main() {
    ...
    let result = match command {
        ...
        Command::Serve => viewer::start(&config.display).and_then(|viewer| {
            tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()
                .map_err(|error| error.to_string())
                .and_then(|runtime| {
                    runtime.block_on(serve::run(App::new(config).with_viewer(viewer)))
                })
        }),
    };
